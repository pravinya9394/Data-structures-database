...
void insert_at_tail(node**head_ref,int data)
{
    ...
    new_node->data=data;                       /*inserting new neode after at tail*/
    node*last=*head_ref;
    new_node->next=NULL;
    if(*head_ref==NULL)
    {
...
bool is_palindrome(node*head)
{
    ...
    {                                            //Time complexity:O(n)
        st.push(temp->data);                     //space complexity:(O(1)
        temp=temp->next;
    }
    while(head!=NULL)
    {
...
{                                                                 // reversing k nodes of linked list
    ...
    return prev;
                            ...
                            // STORE ALL THE ELEMNTS IN HASHTABLE AND THEN WE CAN ACCESS ELEMNTS IN O(1).
